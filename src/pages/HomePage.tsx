...
export const HomePage: React.FC<HomePageProps> = () => {
  ...
  const [newDeliveryDate, setNewDeliveryDate] = useState('September 24, 2026');
  ...
  const handleDirectSubmit = async (e: React.FormEvent) => {
    ...
    if (previewFiles.length === 0) {
      setUploadError('Please select at least 1 exact original screenshot.');
      return;
    }
    setUploading(true);
    setUploadError('');
    ...
    try {
      ...
      previewFiles.forEach((p) => {
        formData.append('screenshots', p.file);
      });
      formData.append('serviceName', newServiceName.trim() || 'Service Activation');
      formData.append('customerId', newCustomerId.trim() || `TC-${Math.floor(1000 + Math.random() * 9000)}`);
      formData.append('customerName', newCustomerName.trim());
      formData.append('deliveryDate', newDeliveryDate.trim() || 'September 24, 2026');
      formData.append('notes', newNotes.trim());

      const res = await fetch('/api/public/add-proof', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!res.ok) {
        ...
        throw new Error(text || 'Failed to upload screenshot');
      }
      ...
      setUploadError(err.message || 'Error uploading screenshots');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-red-600 selection:text-white relative overflow-hidden">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-900/50 border border-slate-800 backdrop-blur-xl shadow-xl">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Quick Add Exact Screenshot Button */}
            <button
              onClick={() => setShowUploadModal(true)}
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-[#4ADE80] hover:text-slate-950 text-[#4ADE80] border border-[#4ADE80]/40 text-xs font-bold transition-all shadow-md cursor-pointer"
            >
              <PlusCircle className="w-4 h-4" />
              <span>Post Exact Screenshot</span>
            </button>
          </div>
        </div>
      ...
      {/* Direct Screenshot Upload Modal - Instant Exact Image Posting */}
      {showUploadModal && (
          ...
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-[#4ADE80]/15 flex items-center justify-center text-[#4ADE80]">
                  <Upload className="w-4 h-4" />
                </div>
                <h3 className="font-bold text-white text-base">Post Exact Original Screenshot</h3>
              </div>
              <button
            ...
            {/* Modal Form */}
            <form onSubmit={handleDirectSubmit} className="p-6 space-y-4">
              {uploadError && (
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-2">
                  Select Original Screenshot Image(s) (JPEG / PNG / WebP) *
                </label>
                <input
                  type="file"
                  ref={fileInputRef}
                ...
                <div
                  ...
                  className="border-2 border-dashed border-slate-700 hover:border-[#4ADE80] rounded-xl p-5 text-center cursor-pointer bg-slate-950/40 hover:bg-slate-900 transition-colors"
                >
                  <ImageIcon className="w-8 h-8 text-slate-500 mx-auto mb-2" />
                  <p className="text-xs font-bold text-white">Click to select original screenshot from your device</p>
                  <p className="text-[11px] text-slate-400 mt-1">Exact, unedited original JPEG/PNG files</p>
                </div>
                ...
                {previewFiles.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {previewFiles.map((p, idx) => (
                      <div key={idx} className="relative group w-16 h-16 rounded-lg overflow-hidden border border-slate-700 bg-black">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Capcut Pro (12 Months)"
                    className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white focus:outline-none focus:border-[#4ADE80]"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">
                    Customer ID / Order ID (Optional)
                  </label>
                  <input
                    type="text"
                    value={newCustomerId}
                    onChange={(e) => setNewCustomerId(e.target.value)}
                    placeholder="Leave empty for random TC-xxxx"
                    className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white focus:outline-none focus:border-[#4ADE80]"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Payment verified and delivered successfully."
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white focus:outline-none focus:border-[#4ADE80]"
                />
              </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  disabled={uploading}
                  className="px-5 py-2 rounded-xl bg-[#4ADE80] hover:bg-[#4ADE80]/90 text-slate-950 text-xs font-black shadow-lg shadow-[#4ADE80]/20 flex items-center gap-2 disabled:opacity-50 cursor-pointer"
                >
                  {uploading ? (
                    <>
